...
function StatusPill({ status }: { status: GoalStatus }) {
  const tone = status === "active" ? "bg-accent-100 text-accent-600" : status === "ready" ? "bg-success-soft text-success" : "bg-secondary text-muted-foreground";
...
export function GoalsSection() {
  ...
  const listRow = (goal: Goal) => {
    ...
    return <button key={goal.id} type="button" onClick={() => setSelectedId(goal.id)} aria-current={selectedId === goal.id ? "true" : undefined} className={`grid w-full cursor-pointer grid-cols-[1.5rem_minmax(0,1fr)_5rem_minmax(0,1fr)_6.5rem] items-center gap-3 rounded-[10px] px-3 py-2 text-left outline-none transition-colors focus-visible:outline-2 focus-visible:outline-ring ${selectedId === goal.id ? "bg-accent-100/70" : "hover:bg-secondary"}`}>
      <span aria-hidden="true" className="text-center text-[15px]">{goal.emoji}</span>
  ...
  return <>
    ...
    <div className="mt-4 rounded-xl border border-line bg-card px-6 py-5">
    ...
    <Dialog open={createOpen} onOpenChange={setCreateOpen}>
      <DialogContent className="sm:max-w-md">
        {step === "suggestion" && !editingId ? <>
          ...
          <div className="rounded-lg border border-primary/30 bg-primary/5 p-3"><p className="m-0 text-[13px] font-medium">🛟 Build an Emergency Buffer</p><p className="m-0 mt-1 text-[12px] leading-relaxed text-muted-foreground">Set aside {formatUSD(800)} each month toward a {formatUSD(10000)} buffer. At the current rate, you can reach it in about 13 months.</p></div>
          ...
          <div className="rounded-lg border border-border p-3"><div className="flex items-center justify-between gap-3"><span className="text-[13px] font-medium">🛟 Emergency Buffer</span><span className="mono text-[13px] font-semibold">{formatUSD(800)} / month</span></div><p className="m-0 mt-2 text-[12px] text-muted-foreground">Main wallet balance will be passively tracked toward a {formatUSD(10000)} target.</p></div>
          ...
          <div className="space-y-3"><div className="space-y-1.5"><label htmlFor="goal-name" className="text-[12px] font-medium text-muted-foreground">Goal name</label><Input id="goal-name" value={name} onChange={(event) => { setName(event.target.value); setError(""); }} placeholder="e.g. Tax Reserve" /></div>
            <fieldset><legend className="mb-1.5 text-[12px] font-medium text-muted-foreground">Emoji</legend><EmojiPickerField value={emoji} onChange={setEmoji} label="Choose a goal emoji" /></fieldset>
